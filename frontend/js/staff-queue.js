...
async function init() {
    try {
        ...
        console.error('!!! CRITICAL: Error initializing queue!', error);
...
async function fetchDepartmentDetails(departmentId) {
    try {
        ...
        departmentData = data; // Full data from backend
        selectedDepartment = { _id: data._id, name: data.name };
        currentPatient = data.currentPatient || null; // Safely assign current patient
        queueData = data.patientQueue || []; // Safely assign patient queue
        ...
        try {
            ...
            console.error('!!! CRITICAL: UI update failed!', uiError);
            throw uiError; // Re-throw to be caught by the outer try-catch
        }
...
function updateDepartmentDisplay() {
    ...
    waitingCountElement.textContent = queueData.length;
    inProgressCountElement.textContent = currentPatient ? 1 : 0;

    if (currentPatient) {
        renderCurrentPatient(currentPatient);
    } else {
        currentPatient = null;
...
function renderCurrentPatient(patient) {
    ...
    currentPatientElement.innerHTML = `
        <div class="patient-info">
            ...
            <p>Age: ${patient.age}</p>
            <p>Priority: <span class="priority-${(patient.priority || 'Normal').toLowerCase()}">${patient.priority || 'Normal'}</span></p>
...
async function handlePatientUpdate() {
    ...
    try {
        ...
        const response = await fetch(`/patients/${selectedPatient._id}/statue`, {
            method: 'POST',
            headers: {
